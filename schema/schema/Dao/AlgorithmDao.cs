...
namespace schema.Dao
{
    public class AlgorithmDao
    {
        ...
        public string GetNxtDeptInfo(string patient_id)
        {
            ...
            int num = unFinishedProjectNum(patient_id);
            if (num > 0)
            {
        ...
        private int unFinishedProjectNum(string patient_id)
        {
            return db.T_QUEUE_LIST.Where(x => (x.PATIENT_ID == patient_id && x.STATUS == null)).ToArray().Length;
        }
        ...
        {
            return db.T_QUEUE_LIST.Where(x => (x.DAPART_CODE == deptcode && (x.STATUS == 0 || x.STATUS == 1))).ToArray().Length;
        }
